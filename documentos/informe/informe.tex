...
\title{\textbf{Trabajo Práctico Nº 1: Conjunto de instrucciones MIPS}}
...
Es función hoja (leaf), porque no llama a otras funciones.
Por ser una función hoja no tiene ABA y la SRA no tiene ra.
El tamaño del stack frame de la función mcd es de 16 words.

\subsection{MCM}

\includegraphics[scale=0.8]{stackmcm}

Función no hoja (non leaf), porque llama a la función MCD.
Por ser una función no hoja tiene ABA, dado que a través de esta área se guardan los parámetros que se utilizan para llamar a otras funciones. La función llamada (invocada) guarda en la ABA de la función que la llamó los parámetros antes de ser utilizados. Además en la saved register area (SRA) de la función llamante se guardará el valor de la dirección a la próxima instrucción al llamado de la función, luego de que la función llamada finalice retornando un valor (en el registro v0) o no.
